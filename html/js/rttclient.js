/*!
 * rttclient.js - A demonstration IM client for Trophy.js
...
SessionController.prototype = {
    connect: function(service, jid, password) {
        ...
        if (this.isConnected) {
            ...
            this.chatboxController = new ChatboxController(this.chatView, this.chatboxSelector,
                    this.connection);
            this.connection.connect(this.jid, this.password, this._onConnect.bind(this));
        }
    ...
    _onConnect: function(status) {
        if (status == Strophe.Status.CONNECTING) {
            ...
            this._finalizeDisconnect();
            if (this.reconnect)
                this.connect(this.service, this.jid, this.password);
            else
...
$(document).ready(function() {
    ...
    $("#cred").on("submit", function() {
        sessionController.connect(BOSH_SERVICE,
                $('#jid').get(0).value,
                $('#pass').get(0).value);
        return false;
    });
